...
def plot_firing_rates(freqs, plot_name, layer_type):
  plt.hist(freqs)
  rm_dir_name = plot_name.split('/')[-1]
  base_name = rm_dir_name.split('.')[0]
...
def plot_weight_changes(plot_name, l23_input_string, l23_recurrent_string, l23):
  """ plot_name should not have directories or extension
  plots input string as histogram on the same figure as l23.inputWeightMatrix
  plots recurrent string as histogram on the same figure as l23.recurrentWeightMatrix
  saves to file: plots/plot_name-compare-pre-post-[input/rec].png
  """
  ...
  plt.savefig("plots/" + plot_name + "-hist-compare-pre-post-input.png")
  plt.close()
  ...
  plt.savefig("plots/" + plot_name + "-scatter-compare-pre-post-input.png")
  plt.close()
  ...
  plt.savefig("plots/" + plot_name + "-hist-compare-pre-post-rec.png")
  plt.close()
  ...
  plt.savefig("plots/" + plot_name + "-scatter-compare-pre-post-rec.png")
  plt.close()

def weights(layer=None):
  # reads a weight matrix filename saved by the model using tofile
  # plots a histogram weights
